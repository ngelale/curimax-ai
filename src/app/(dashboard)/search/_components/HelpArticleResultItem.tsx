"use client";

import React from "react";
import { ArrowRight, BookMarked, Star } from "lucide-react";
import { HelpArticleResult } from "../types";
import { Button } from "@/app/components/ui/button";

interface HelpArticleResultItemProps {
  result: HelpArticleResult;
  onRead: (articleId: string) => void;
}

export function HelpArticleResultItem({
  result,
  onRead,
}: HelpArticleResultItemProps) {
  return (
    <div className="border border-gray-200 rounded-lg p-6 hover:border-blue-300 hover:shadow-md transition-all bg-white">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <BookMarked className="w-5 h-5 text-green-600" />
            <h3 className="text-lg font-semibold text-gray-900 hover:text-blue-600 cursor-pointer">
              {result.title}
            </h3>
          </div>
          <div className="flex items-center gap-3 text-xs text-gray-600">
            <span className="px-2 py-0.5 bg-green-50 text-green-700 rounded font-medium">
              {result.category}
            </span>
            <span>•</span>
            <span>{result.readTime} min read</span>
            <span>•</span>
            <div className="flex items-center gap-1">
              <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
              <span>{result.rating}</span>
            </div>
          </div>
        </div>
        <div className="text-right">
          <div className="text-sm font-semibold text-gray-900">
            {result.views}
          </div>
          <div className="text-xs text-gray-500">views</div>
        </div>
      </div>

      <p className="text-gray-700 text-sm mb-4">{result.description}</p>

      <div className="flex items-center justify-end">
        <Button
          onClick={() => onRead(result.id)}
          variant="ghost"
          size="sm"
          className="text-blue-600 hover:text-blue-700 gap-1"
        >
          Read Article <ArrowRight className="w-4 h-4" />
        </Button>
      </div>
    </div>
  );
}
